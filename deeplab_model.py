...
n_classes = 2 ## classification number

## training stage
training_stage_index = 0

#
training_stage_index_placeholder = tf.compat.v1.placeholder(tf.int16)
...
class DeepLabLFOVModel(object):
    def __init__(self, weights_path=None):
        ...
        if weights_path is not None:
            self.trained_weights = np.load(weights_path,allow_pickle=True,encoding="latin1").item()
        else:
    ...
    def _get_weights_and_biases(self,name,shape=None,trainable=True):
        if name.startswith("stack"): # the weights for baseline network
            ...
            if stack_index < 5:
                with tf.compat.v1.variable_scope("variable",reuse=tf.compat.v1.AUTO_REUSE) as scope:
                    w,b = self._restore_weights_and_biases_for_pretrain(name)
                    init_w = tf.constant_initializer(w)
                    shape_w = w.shape
                    w = tf.compat.v1.get_variable(name="%s_w"%name,initializer=init_w,shape=shape_w)
                    init_b = tf.constant_initializer(b)
                    shape_b = b.shape
                    b = tf.compat.v1.get_variable(name="%s_b"%name,initializer=init_b,shape=shape_b)
        ...
        with tf.compat.v1.variable_scope("variable",reuse=tf.compat.v1.AUTO_REUSE) as scope:
            w = tf.compat.v1.get_variable(name="%s_w" % name,shape=shape,initializer=tf.contrib.layers.xavier_initializer_conv2d(dtype=tf.float32))
            b = tf.compat.v1.get_variable(name="%s_b" % name,shape=[shape[-1]],initializer=tf.contrib.layers.xavier_initializer_conv2d(dtype=tf.float32))
        self.weights[name] = [w,b]
        self.l2_loss += tf.nn.l2_loss(w)
        return w,b
        ...
        with tf.name_scope("stack%d" % stack_index) as scope:
            ...
            if pool_size is not None:
                with tf.name_scope("pool") as scope:
                    current = tf.nn.max_pool2d(current,ksize=[1,pool_size,pool_size,1],strides=[1,pool_stride,pool_stride,1],padding="SAME")
                    if stack_index == 4:
                        current = tf.nn.avg_pool2d(current,ksize=[1,pool_size,pool_size,1],strides=[1,pool_stride,pool_stride,1],padding="SAME")
    ...
    def prepare_label(self, input_batch, new_size):
        with tf.name_scope('label_encode'):
            input_batch = tf.compat.v1.image.resize_nearest_neighbor(input_batch, new_size[1:3]) # As labels are integer numbers, need to use NN interp.
    ...
    def loss(self,img_batch,label_batch):## loss function
